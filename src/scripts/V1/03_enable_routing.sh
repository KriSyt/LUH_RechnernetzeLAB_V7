...
### NodeB
sshpass $SSHP_ARGS ssh $SSH_ARGS  root@nodeB "sudo sysctl -w net.ipv4.conf.all.accept_redirects=1 > /tmp/enableRouting_nodeB.log"
sshpass $SSHP_ARGS ssh $SSH_ARGS -f root@nodeB "sudo sysctl -w net.ipv4.conf.$nodeB_if.accept_redirects=1 >> /tmp/enableRouting_nodeB.log"
